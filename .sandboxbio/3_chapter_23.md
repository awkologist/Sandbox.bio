...
Now we analyze (allmost) all files:
...
```bash
grep -c "SPIKE" result.txt
```

```bash
grep -c "Motif" result.txt
```

```
grep "Motif" result.txt | head -5
```

```bash
grep "Motif" result.txt | cut -d ' ' -f 7 | sort | uniq -c
```
...
Now paste the code generated by the following programme to Jmol:

```bash
grep "Motif" result.txt | awk '{print $6}' | sort | uniq -c | sed 's/:/ /' | awk '{if($1<10){print "select "$3":B; spacefill 100; color yellow"}else{print "select "$3":B; spacefill 300; color yellow"}}' | tee -a 7DF4.script
```
